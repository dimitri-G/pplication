...
public class GoodsItem{
    ...
    private static void initData(final String shopid){
        ...
        {new Thread(new Runnable() {
            ...
            public void run() {
                ...
                int typeid=0;
                int flag=0;
                for(int i = 1; i < (int)spite.length/4; i++){
                    int price=Integer.parseInt(spite[1+i*4]);
                    typeid=Integer.parseInt(spite[2+i*4]);
                    item = new GoodsItem(i,price,spite[4*i],typeid,spite[4*i+3]);
                    goodsList.add(item);
                    if(flag!=typeid)
                    {
                        typeList.add(item);
    ...
    public static ArrayList<GoodsItem> getGoodsList(String shopid){
        if(goodsList==null){
            initData(shopid);
        }
        return goodsList;
    }
    public static ArrayList<GoodsItem> getTypeList(String shopid){
        if(typeList==null){
            initData(shopid);
        }
        return typeList;
    ...
    public static String postwebinfo(String shopid){
        ...
        try {
            ...
            //1,找水源--创建URL
            URL url = new URL("http://192.168.43.21:8080/TomcatTest/BuyyingServlet");//放网站
